...
authRouter.post('/signup', async (req, res) => {
  ...
  const { password, timezone } = parsed.data;
  ...
  const id = randomUUID();
  db.prepare(
    `INSERT INTO users (id, email, password_hash, created_at, timezone)
     VALUES (?, ?, ?, ?, ?)`,
  ).run(id, email, await bcrypt.hash(password, BCRYPT_COST), new Date().toISOString(), timezone ?? 'UTC');
